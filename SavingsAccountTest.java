...
public class SavingsAccountTest {
    ...
    // ── Generated by /generate-tests CorporateSavingsAccount (precise prompt) ──────────
    private static void testCorporateAccountEdgeCases() {
        // Happy-path: confirm all five core assertions for a mid-range balance
        SavingsAccount account = new CorporateSavingsAccount("  ACME Corp  ", bd("100000.00"));

        assertEquals("ACME Corp", account.getAccountName(), "账户名应自动去除首尾空白");
        assertEquals(CustomerType.CORPORATE, account.getCustomerType(), "应为企业客户类型");
        assertEquals(CorporateSavingsAccount.INTEREST_RATE, account.getAnnualInterestRate(), "企业年利率应为3.50");
        // 100000 * 3.50 / 100 = 3500.00
        assertEquals(bd("3500.00"), account.calculateInterest(), "企业客户利息: 100000 × 3.50%");
        assertEquals(bd("103500.00"), account.calculateNewBalance(), "企业客户新余额: 本金 + 利息");

        // Error path 1: null account name must throw
        try {
            new CorporateSavingsAccount(null, bd("1000.00"));
            fail("null 账户名应抛出 IllegalArgumentException");
        } catch (IllegalArgumentException e) {
            assertExceptionContains(e, "账户名称不能为空");
            System.out.println("  ✓ 企业账户 null 名称异常 — 捕获: " + e.getMessage());
            passed++;
        }

        // Error path 2: balance exceeding MAX_BALANCE must throw
        try {
            new CorporateSavingsAccount("溢出测试", SavingsAccount.MAX_BALANCE.add(bd("0.01")));
            fail("超过最大余额应抛出 IllegalArgumentException");
        } catch (IllegalArgumentException e) {
            assertExceptionContains(e, "原始余额超过最大限额");
            System.out.println("  ✓ 企业账户超额异常 — 捕获: " + e.getMessage());
            passed++;
        }

        System.out.println("✓ testCorporateAccountEdgeCases 通过");
        passed++;
    }
    // ─────────────────────────────────────────────────────────────────────────────────────────
